 ...
 */
@Component( {
    selector: 'page-prodetail',
    templateUrl: 'prodetail.html',
  ...
  totalct:any
   ...
   this.commonservice.getBranchEmployees(this.empData['id_branch']).then(data=>{
    ...
    openProductdetails( id_design ) {
        this.navCtrl.push( ProdetailPage, { proid: id_design } );
    }
...
(err) => {
  ...
  console.log(err);
    ...
    presentAlert(title, message) {
      ...
      checkstock(id){
        ...
        this.ids = id;
        this.commonservice.checkstock({'id_branch': empData['id_branch'],'id_weight':this.idw ,'id_size':id ,'id_category':  this.productdet['product_id'],'id_product':  this.productdet['product_id'],"design_no" :  this.productdet['design_id'],'id_sub_design': this.productdet['id_sub_design'], "tag_id": this.productdet['id_tag_detail'],"tag_code": this.productdet['TagNo']}).then(result => {
        ...
        checkwgstock(id){
          ...
          this.commonservice.checkwgstock({'id_branch': empData['id_branch'],'id_size':this.ids , 'id_weight':id,'id_category':  this.productdet['product_id'],'id_product':  this.productdet['product_id'],"design_no" :  this.productdet['design_id'],'id_sub_design': this.productdet['id_sub_design'], "tag_id": this.productdet['id_tag_detail'],"tag_code": this.productdet['TagNo']}).then(result => {
  ...
  public uploadvideo(path,name) {
    ...
    this.fileTransfer.upload( path, encodeURI(url), options ).then(( data ) => {
      console.log(data);
...
// });
  ...
  addStone(){
    ...
    let modal = this.modal.create(Addstone2Page,{"stoneMasData":this.stoneMasData, "action":"add" , "stone_details":this.stone_details,"ptype":'home_bill','gross':gross });
    modal.present();
  ...
  createEsti(){
    ...
    this.navCtrl.push(EstiPage,{"page_type" : 'create','ecat':this.productdet.TagNo,'bcode':this.productdet.BranchCode});
  ...
  downloadfile(url,name) {
    ...
    this.platform.ready().then(() =>{
      ...
      if(this.platform.is('android')) {
        ...
        this.file.checkDir(this.file.externalDataDirectory, this.commonservice.cmpShortName).then((data) => {
          console.log("Check directory IDO");
          this.file.checkDir(this.file.externalDataDirectory+''+this.commonservice.cmpShortName+'/', date.toISOString().split('T')[0]).then(_ => {
            console.log("Check directory IDO "+date.toISOString().split('T')[0]);
            this.viewFile(url,name);
          }).catch(er => {
              this.file.createDir(this.file.externalDataDirectory+''+this.commonservice.cmpShortName+'/', date.toISOString().split('T')[0],false).then(_ => {
          ...
          this.file.createDir(this.file.externalDataDirectory, this.commonservice.cmpShortName,false).then(_ => {
  ...
  viewFile(url,name){
    ...
    this.platform.ready().then(() => {
      ...
      this.file.resolveDirectoryUrl(this.file.externalDataDirectory+''+this.commonservice.cmpShortName+'/'+date.toISOString().split('T')[0]).then((resolvedDirectory) => {
        ...
        this.file.checkFile(resolvedDirectory.nativeURL, name).then((data) => {
          ...
          var targetPath = this.file.externalDataDirectory+''+this.commonservice.cmpShortName+'/'+date.toISOString().split('T')[0]+'/'+name;
