...
// @IonicPage()
@Component({
  selector: 'page-subdesignlist',
  templateUrl: 'subdesignlist.html',
  providers: [CommonProvider,RetailProvider]

})
export class SubdesignlistPage {
  ...
  hbdesignSelected(){ 
    // if(this.hbDesigns.length == 0) {
      ...
      loader.present();
      this.comman.getsubdesigns({"id_product":this.hbData['pro_id'], "design_no":this.hbData['design_no']}).then(data=>{
  ...
  editdesign(data){
    ...
    this.navCtrl.push(SubdesignimgPage,{'data':data})
  }
  search() {

    
    
    this.hbsubDesigns = this.searchhbsubDesigns.filter(item => item['label'].toUpperCase().includes(this.input.toUpperCase()));
  }
}
