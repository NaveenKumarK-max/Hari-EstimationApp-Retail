...
// @IonicPage()
@Component({
  selector: 'page-subdesignimg',
  templateUrl: 'subdesignimg.html',
  providers: [CommonProvider,RetailProvider]

})
export class SubdesignimgPage {
 ...
 private copyFileToLocalDir( namePath, currentName, newFileName,index,type ) {
  ...
  public uploadImage(name) {
    ...
    let imgs:any[] =  this.images.filter(data=>!data.hasOwnProperty('id_design_img'));
    ...
    imgs.forEach(async (element,index) => {
      ...
      // Destination URL
      var url = BaseAPIURL+'admin_app_api/createSubDesignImage';
      var d = new Date(),
      n = d.getTime()
    // File for Upload
    ...
    var options = {
      ...
      params: {
        ...
        id_sub_design:this.whole['id_sub_design'],
        branch_id:this.empData['id_branch'],
        created_by:this.empData['uid'],
        id_sub_design_mapping:this.whole['id_sub_design_mapping']
      }
    };
    ...
    fileTransfer.upload( targetPath, encodeURI(url), options ).then(( data ) => {
      ...
      this.images[check]['id_design_img'] = result['imageid'];
...
// });
        ...
        buttons: [
            {
                ...
                handler: () => {
                  ...
                  if(this.images[index].hasOwnProperty('id_design_img')){
                      ...
                      this.comman.deletegallery(temp).then(data=>{
  ...
  checkdefault(index){
    ...
    if(this.images[index].hasOwnProperty('id_design_img')){
      ...
      this.comman.updatedefault(temp).then(data=>{
        let toast = this.toast.create({
